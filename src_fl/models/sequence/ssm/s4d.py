...
class SSKernelDiag(nn.Module):
    ...
    def __init__(
        ...
        **kwargs # For compatibility with other kernels
        ...
        self.register("log_dt", log_dt, train_dt, lr, 0.0)

        log_w_real = torch.log(-w.real + 1e-4)
        w_imag = w.imag
        self.register("log_w_real", log_w_real, train_w, lr, 0.0)
        self.register("w_imag", w_imag, train_w, lr, 0.0)
    ...
    def forward(self, L):
        ...
        K = 2*K.real
        # Keops version is more memory efficient
        # C = C * (torch.exp(dtA)-1.) / w
...
class S4D(nn.Module):
    ...
    def forward(self, u, **kwargs): # absorbs return_output and transformer src_fl mask
        """
